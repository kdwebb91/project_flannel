...
namespace Flannel
{
    class Program
    {
        ...
        /*static void Main(string[] args)
        {
            ...
            List<string> artistAuth = Hits.SortByAuthScore(artistIds);
            List<string> artistHub = Hits.SortByHubScore(artistIds);
            List<string> artistFam = Metadata.SortByArtistFamiliarity(artistIds);
            Console.WriteLine("AUTH\t\tHUB\t\tFAM");
            for (int i = 0; i < artistHub.Count; i++)
            {
                Console.WriteLine(Metadata.getArtistNameFromId(artistAuth[i]) + "\t" + Metadata.getArtistNameFromId(artistHub[i]) + "\t" + Metadata.getArtistNameFromId(artistFam[i]));
            }

            //Rec.GeneratePlaylist(Artists);
            Console.Read();
        }*/
    }
}
